...
public class AirportTest {
    ...
    @Test
    public void testSortingAccuracyMaximumLoadCapacity() {
        Airport airport = new Airport(planes).sortByMaxLoadCapacity();
        for (int i = 0; i < airport.getPlanes().size() - 1; i++) {
            Assert.assertTrue(airport.getPlanes().get(i).getMaxLoadCapacity()
    ...
    public void testHasAtLeastOneBomberInMilitaryPlanes() {
        ...
        for (MilitaryPlane militaryPlane : airport.getBomberMilitaryPlanes()) {
            Assert.assertFalse(militaryPlane.getMilitaryType() == MilitaryType.BOMBER);
        }
